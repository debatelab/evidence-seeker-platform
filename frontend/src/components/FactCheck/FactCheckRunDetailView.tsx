...
interface FactCheckRunDetailViewProps {
  data: PublicFactCheckRunDetailResponse | null;
  loading: boolean;
  error: string | null;
  backHref: string;
  onShare?: () => void;
  ...
  seekerOverride?: Partial<PublicEvidenceSeekerSummary>;
}

const FactCheckRunDetailView: React.FC<FactCheckRunDetailViewProps> = ({
  data,
  loading,
  error,
  backHref,
  ...
  if (loading) {
    return (
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center text-gray-500">
        Loading fact check...
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
        ...
        <Link to={backHref} className="mt-4 btn-primary px-5 py-3 text-base">
          Back
        </Link>
      </div>
    );
  ...
  const seekerMeta: PublicEvidenceSeekerSummary = {
    ...seeker,
    ...seekerOverride,
  };
  const submittedAt = formatRelativeTime(run.createdAt);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-6">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-gray-500">
              Powered by{" "}
              <Link
                to={`/evidence-seekers/${seekerMeta.uuid}`}
          ...
          {run.visibility === "UNLISTED" && (
            <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-amber-900">
              ...
              <div>
                ...
                <p className="text-sm">
                  This fact check is unlisted. Only people with this link can view it.
                </p>
              </div>
            </div>
          )}
...
export default FactCheckRunDetailView;
